...
public class SessionPacketHandler implements PacketHandler {
    ...
    public void processPacket(ByteBuffer byteBuffer) {
        if (packetFormat > 0) {
            ...
            boolean isSameSessionRestart = (sd.sessionTimeLeft() > this.sessionInformationWrapper.getSessionTimeLeft() && !this.sessionTimeBuffer.contains(sd.sessionTimeLeft()));            //If the sessionInformation objects are not equal OR the session time left on the latest session data packet is greater than the session information's time left param.
